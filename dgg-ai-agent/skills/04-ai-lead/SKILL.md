...
接企业自己的成交记录与线索来源（CRM、订单、展会扫码、官网留资、公众号、老客户转介绍、名录导入），一次跑出三样东西并串成闭环：**画像**（从成交客户反推谁是好客户）、**脚本**（按画像与渠道分阶段的话术）、**线索**（按画像打分排序、解释、分派）、**跟进**（本周计划、逾期、成交预测、周报）。纯预制、无 LLM、断网可用；画像权重、脚本采用、分派、计划都写进同一份数据副本，各屏按副本重算。
...
| `plan / funnel / forecast / report` | 本周计划（今天起 5 个工作日，每组每天 ≤ 5 条）、五级漏斗与渠道投入产出、按阶段概率加权的成交预测、周报文本 |

## 口径

- 线索来源只用企业自有渠道加一个不带厂商名的「名录导入」；全文不出现任何数据源厂商名。
