...
| `diagnosisDays` | 「要不要安排专家入企 AI 诊断？1980 元一天。不安排也可以。」 | **可不填**：按场景数量分档（1–2 个场景 1 天、3–5 个 2 天、6 个及以上 3 天），取自 `investment-profile.json` 的 `diagnosisDays.values` |
...
| 计算项 | 规则 |
|---|---|
| 投入 · 现金 | 订阅 = 版本年费 × 套数（第 1、13 个月整付）；入企诊断 = 1980 × 天数；私域 = 端点价（12800 或 33800）× 1 套，按一套部署计列、不乘账号数；另议项按填写值。一次性项只在第 1 个月计 |
...
`buildPrompt(result, promptTemplate)` 把三段文字填进 `prompts/polish.md`：`{verdict}` 取 `verdict.headline`，`{costLine}` 取 `quickView[0].text`（测算范围句），`{gainLine}` 取 `quickView[2].text`（投入结构句）——两处取的是写死的下标，进模板的并不是月度收益句与投入总额句。

模板要求 LLM 按 `{verdict, costLine, gainLine}` 返回，但 `mergePolish(result, polished)` 当前只把 `polished.verdict` 回写到 `verdict.headline`，`costLine` 与 `gainLine` 收到后不落地，`quickView` 各条仍是模板原文。`polished` 为空时结果原样返回；该函数就地改写传入对象，平台侧须先深拷贝再把返回值写回会话状态。

任何情况下都不改数字。超时、返回不可解析或命中禁忌词就用模板文案，标「初稿」。
...
| render.type | 数据 | 呈现 |
|---|---|---|
| `portfolio` | `portfolio` | 本次立项的组合概览：场景数、每批场景数、实施批次与总工期 |
...
| `scene-contribution` | `portfolio.scenes` | 逐场景的月度现金效益与占全组合比重 |
| `wave-gantt` | `portfolio.waves` | 实施波次：每批的场景、起止周与起算期次 |
