...
| `click` | `aim` | 替用户按下这一屏上写着这几个字的按钮 | 按 §12.1 的顺序与匹配规则找到那个按钮并点下去；界面上没有按钮的平台忽略即可 |
...
- `click` 是**界面层**的动作，只有「有按钮可点」的宿主（展台、Web 控制台）才有意义；命令行、MCP、HTTP 这类没有界面的平台一律忽略。`aim` 写按钮上的字，不写选择器、不写 id。它不改数据，所以不受上面的防递归三道校验约束，但同样只展开一跳。**怎么找到那个按钮见 §12.1。**
...
### 12.1 `click` 怎么找到那个按钮（1.1 澄清）

`aim` 只给「按钮上写的字」，怎么在界面上定位到它由宿主决定。1.1 把展台宿主实跑踩出来的规则写进规范 —— 不是新字段、不是新语义，是把「最小实现」说清楚，照着做的平台能少走两轮弯路。

**找的顺序（三处，依次）**

1. **浮层**：抽屉 / 弹窗 / 模态开着的时候，该点的一定在浮层里。
2. **当前屏工作区**：这一屏的主体内容。
3. **整屏**：包括常驻的底栏、页头、侧栏 —— 屏底那条「下一步」通常挂在工作区外面，只搜工作区会找不到。

**匹配规则**

- 先在三处依次做**前缀匹配**；三处都没命中，再按**包含**（`aim` ≥ 2 字）走一遍同样的三处顺序。
- 比较前把两边的空白全去掉：按钮上常有 `下一步 · 提交` 这种带空格与间隔号的写法。
- **跳过禁用的与不可见的**（`disabled`、不在渲染树里、被折叠的）。
- 命中多个取第一个；一个都没命中就当这条 `act` 不存在，**静默忽略，不报错**。

**两条踩出来的坑**

- 浮层开着的时候去指屏底那条，隔着一层点不到 —— 所以浮层优先级必须最高。
- 屏底常驻的那条不在工作区里，只搜工作区必然落空 —— 所以第三处要搜整屏。

**给界面宿主的建议（SHOULD，不是 MUST）**：先把按钮高亮一下再按下去，让人看见「AI 动的是哪一下」。展台宿主的做法是高亮后隔约 0.5 秒再触发点击。
