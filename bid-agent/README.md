...
`standalone/标书生成智能体.html` 是把核心流水线移植到浏览器端的**单文件版**：
无需 Python、不联网、双击即可使用，适合快速体验与分享。支持粘贴/上传文本招标文件、
内置管道行业示例知识库、抽取废标项与评分、生成标书并导出 Word(.doc)。
（完整功能 —— .docx/.pdf 解析、向量混合检索、本地大模型生成 —— 仍以上面的 Python 版为准。）
