/* ===== 陪练关卡定义（两份，反馈与判定文案照客户方案原文；教练台词为监护人 / 值班负责人口吻） =====
   ...
   两种情境：type 'quiz' 做题（items：choice 选择 / fill 填空 / qa 问答，教练即时回应）；操作关（默认）——task 本关任务、guide 屏幕上的怎么做（g 对应 goal，t 要点，how 训练模式才显示的具体做法），
   goals 是学员要做到的事：look 走到设备前看并说出状态 · op 按住 / 拖动才算操作 · say 口述结论或汇报 · drag 拖动手柄；做完当场打分。分值：操作关沿用评分表权重（1163 十分、雨淋阀十分），做题另加分。 */
...
const SP1163 = {
  hmi: { cb: { n: '培训三线1163开关', zoom: zoomHmiCb, bad: '这是 1163 开关本身。' }, es: { n: '116340 地刀', zoom: st => zoomHmiRows(st, 'volt').replace('遥测 · 培训三线 1163 线路二次电压', '116340 地刀 · 后台位置'), bad: '这是 116340 地刀的后台位置，不是你现在要看的。' }, cur: { n: '三相电流遥测', zoom: st => zoomHmiRows(st, 'cur'), bad: '这是三相电流。' }, volt: { n: '线路二次电压遥测', zoom: st => zoomHmiRows(st, 'volt'), bad: '这是线路二次电压。' }, sig: { n: '信号列表', bad: '信号列表可以看，但结论要从位置和遥测来。' } },
  mech: { ind: { n: '分、合闸指示窗', zoom: zoomMech }, manual: { n: '手动分合闸按钮', bad: '停，机构箱里的手动按钮不能碰，这一项只看不动。', crit: true, fix: '检查项只看不动，不得触碰机构箱内操作按钮', risk: RISK_ID, rule: RULE_CHK } },
  cab: { esInd: { n: '就地控制柜 116340 位置指示', zoom: zoomEsInd }, hv: { n: '高压带电显示装置', zoom: zoomHv }, cbInd: { n: '1163 开关位置指示', bad: '这是 1163 开关的指示，不是地刀。' } },
  gnd: {},
  pano: { cab: { n: '就地控制柜', zoom: zoomEsInd }, mech: { n: '刀闸机构箱', zoom: zoomMechEs }, rod: { n: '刀闸连杆', zoom: zoomRod } }
};
...
const EXAM_1163 = {
  ...
  bg: '基于「将 110kV 培训三线 1163 线路由运行转检修」操作票简化，只保留两个关键能力：断开开关后的状态检查；验电、接地及接地后的状态检查。做题与操作交替：选择 / 填空 / 问答由监护人即时回应；操作关先在屏幕上给出怎么做，做完当场打分。中间非重点步骤由系统完成。',
  max: 15, pass: 9, crit: 3, cover: ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H'],
  ...
  stations: [
    { id: 'q0', type: 'quiz', part: '关卡一 · 断开开关前', title: '答题 · 操作前核对', dims: { C: 1, H: .5 }, locs: { hmi: svgHmi1163 }, locNames: { hmi: '监控后台' }, spots: {},
      brief: '任玲玲，动手之前先答两道题，答对了再去后台。',
      items: [
        { id: 'c1', kind: 'choice', pts: .5, q: '接到调度令「将培训三线 1163 线路由运行转检修」，在后台断开 1163 开关之前，首先要核对的是？', opts: ['后台一次接线图上的运行方式，以及 1163 间隔开关、刀闸位置和光字、告警、报文', '直接在一次接线图上遥控分闸，分完再看', '先到现场看 1163 开关本体', '先合上 116340 地刀做安全措施'], a: 0, why: STEPS.find(x => x.no === '2').why, rule: STEPS.find(x => x.no === '2').rule, hint: '操作票第二项写的是什么？' },
        { id: 'f1', kind: 'fill', pts: .5, q: '遥控操作前应核对设备___和操作___，预置返校正确后再执行。', blanks: [[/双重名称|双重称号|双重编号|双重名/], [/性质/]], sample: '双重名称，性质', why: '遥控对象与操作性质核对是后台操作防误的第一道关：对象错了会误操作别的间隔，性质错了会把分闸做成合闸。', rule: '附录F 2.11.3：按操作票项目的操作性质执行，遥控操作前核对设备双重名称与操作性质。', hint: '遥控操作框里要你口述的两样东西。' }
      ],
      outro: '好，去后台。' },
    { id: 'k1_open', task: '在监控后台断开培训三线 1163 开关', guide: [{ g: 'open', t: '在后台分图一次接线上找到 1163 开关，核对双重名称', how: '点 1163 方块进入遥控操作' }, { g: 'open', t: '按住「分闸」直到执行完成，看变位报文', how: '松手作废；执行后信号栏出现「1163 开关 分闸 变位」' }], part: '关卡一 · 断开开关后的状态检查', title: '断开培训三线 1163 开关', dims: { C: 1, H: .5 }, locs: { hmi: svgHmi1163 }, locNames: { hmi: '监控后台' }, spots: SP1163.hmi,
      brief: '任玲玲，调度令已经下了：将 110kV 培训三线 1163 线路由运行转检修。第一项，断开培训三线 1163 开关。你来做，做的时候手指口述。',
      goals: [{ id: 'open', kind: 'op', n: '断开培训三线 1163 开关', dev: ['cb'], loc: 'hmi', op: 'cb:open', re: /断开|分闸|拉开/, pts: .5, act: st => { st.cb = 'open'; }, praise: '分闸命令发出，后台报文：1163 开关 分闸 变位。', hints: ['开关在后台分图的一次接线上。', '点开 1163 开关，用遥控操作分闸。', '点一次接线上的 1163 方块，按住「分闸」直到执行完成。'], nudge: '任玲玲，第一项还没做。' }],
    ...
    { id: 'k1_check', task: '三处独立确认 1163 开关已可靠分闸，再下结论', guide: [{ g: 'hmiPos', t: '看后台 1163 开关位置遥信', how: '点一次接线上的 1163 方块，说出分闸 / 合闸' }, { g: 'mech', t: '到现场看 1163 开关机构箱分、合闸指示', how: '切到「1163 开关机构箱」，点指示窗，读出分 / 合' }, { g: 'cur', t: '看后台三相电流', how: '点遥测信息栏 Ia / Ib / Ic，把读数说出来' }, { g: 'concl', t: '三处一致后下结论', how: '说：三处一致，确认 1163 开关已可靠分闸' }], part: '关卡一 · 断开开关后的状态检查', title: '确认 1163 开关可靠分闸', dims: { B: 1.5, H: .5, F: 1, A: .5 }, locs: { hmi: svgHmi1163, mech: svgMechBox }, locNames: { hmi: '监控后台', mech: '1163 开关机构箱（现场）' }, start: 'hmi', spots: Object.assign({}, SP1163.hmi, SP1163.mech), rule: RULE_CHK, risk: RISK_GIS,
    ...
    { id: 'q1', type: 'quiz', part: '关卡一 · 答题', title: '答题 · 确认分闸', dims: { B: 1, C: 1 }, locs: { hmi: svgHmi1163 }, locNames: { hmi: '监控后台' }, spots: {},
      brief: '开关分开了。两道题，答完进第二关。',
      items: [
        { id: 'qa1', kind: 'qa', pts: 1, q: '为什么执行了分闸操作，不等于已经确认分闸成功？要说到你刚才确认了哪几处、为什么要确认。', keys: [{ t: '后台位置遥信', re: /后台|监控|遥信|画面/ }, { t: '机构箱分合闸指示', re: /机构箱|机械|指示/ }, { t: '三相无电流', re: /电流/ }, { t: '开关可能拒动或未到位，要确认实际状态', re: /实际|状态|拒动|没到位|未到位|可靠|真的/ }], need: 3, sample: '分闸命令发出后，开关可能拒动或没到位，所以要看后台位置、现场机构箱的机械指示和三相电流，三处都确认了，才能说开关实际已经可靠分闸。', sampleBad: '因为规程要求。', right: '后台位置、机构箱机械指示、三相电流三处独立确认，因为开关可能拒动或未到位', why: '命令只是发出去了，开关到没到位要看结果指示。', rule: RULE_CHK, hint: '想想刚才你确认了哪三处，开关为什么可能分不开。' },
        { id: 'c2', kind: 'choice', pts: .5, q: '下列哪一项不能作为「1163 开关已分闸」的独立确认依据？', opts: ['后台位置遥信显示分闸', '机构箱分、合闸指示显示分', '三相电流遥测均为零', '分闸命令已经发出的操作记录'], a: 3, why: '命令发出不等于开关到位，开关可能拒动或未到位；确认要看结果指示，不看命令记录。', rule: RULE_CHK, hint: '哪一项只说明"做了"，不说明"到位了"？' }
      ],
      outro: '对了。第二关。' },
    ...
    { id: 'q2', type: 'quiz', part: '关卡二 · 接地前', title: '答题 · 验电要求', dims: { D: 1, C: .5 }, locs: { hmi: svgHmi1163 }, locNames: { hmi: '监控后台' }, spots: {},
      brief: '线路已经转到冷备用，五防钥匙也下传了。合地刀之前先答两道题。',
      items: [
        { id: 'c3', kind: 'choice', pts: 1, q: 'GIS 间隔不能直接验电时，合接地刀闸前的验电要求是？', opts: ['两个及以上非同样原理或非同源的指示且均已同时发生变化', '只看后台线路二次电压为零即可', '只看高压带电显示装置灯灭即可', '由监护人口头确认线路已停电即可'], a: 0, why: '单一指示可能失效或未刷新，两种非同源指示同时变化才能判定确无电压。', rule: RULE_VD2, hint: '规程要几种原理？' },
        { id: 'f2', kind: 'fill', pts: .5, q: '本站 1163 间隔的两种验电指示：后台线路___电压，以及就地控制柜的高压___显示装置。', blanks: [[/二次/], [/带电/]], sample: '二次，带电', why: '一个来自后台遥测，一个来自现场装置，原理不同、来源不同。', rule: RULE_VD2, hint: '一个在后台遥测栏，一个在就地控制柜上。' }
      ],
      outro: '记住这个要求，动手。' },
    { id: 'k2_hub', task: '两种原理验电确无电压后，合上培训三线线路侧 116340 地刀', guide: [{ g: 'volt', t: '第一种原理：后台线路二次电压', how: '点遥测信息栏 Uab / Ubc / Uca，读数说出来' }, { g: 'hv', t: '第二种原理：就地控制柜高压带电显示装置', how: '切到「就地控制柜」，点带电显示装置，看三相灯灭没灭' }, { g: 'gnd', t: '两项验电都确无电压后，合上 116340 地刀', how: '切到「地刀操作面板」，按住「合闸」到进度走满' }], part: '关卡二 · 验电、接地及接地后的状态检查', title: '验电与接地', dims: { D: 1.5, F: 1, H: .5, B: .5 }, locs: { hmi: svgHmi1163, cab: svgCabFace, gnd: svgGroundPanel }, locNames: { hmi: '监控后台', cab: '就地控制柜', gnd: '地刀操作面板' }, start: 'hmi', spots: Object.assign({}, SP1163.hmi, SP1163.cab, SP1163.gnd), rule: RULE_VD, risk: RISK_VD, fix: '先验电再接地',
    ...
    { id: 'k2_chk', task: '三处检查 116340 地刀位置并判定', guide: [{ g: 'cab', t: '看就地控制柜 116340 位置指示', how: '点全景左侧的就地控制柜，看红灯亮没亮' }, { g: 'mech', t: '看刀闸机构箱分、合闸指示', how: '点全景右下的机构箱，读指示窗' }, { g: 'rod', t: '看刀闸连杆是否到合闸限位', how: '点机构箱旁的连杆，看是否与合闸限位线重合' }, { g: 'concl', t: '三处一致判定合闸到位；不一致则不能确认，中止上报', how: '说出你的判定' }], part: '关卡二 · 验电、接地及接地后的状态检查', title: '116340 地刀合上后的状态检查', dims: { B: 1.5, A: .5, G: 1, E: .5 }, locs: { pano: svgBayPanorama, cab: svgCabFace }, locNames: { pano: '现场全景', cab: '就地控制柜柜面' }, start: 'pano', spots: Object.assign({}, SP1163.pano, SP1163.cab), rule: RULE_GIS, risk: RISK_GIS,
      ...
      outro: '好。现在向我正式汇报一遍。' },
    { id: 'k2_talk', task: '向监护人汇报 116340 地刀检查结果', guide: [{ g: 'rep', t: '汇报要素：设备双重名称、控制柜 / 机构箱 / 连杆三处指示、结论', how: '设备编号按位报读：一一六三四零' }], part: '关卡二 · 口述汇报', title: '向监护人汇报 116340 地刀检查结果', dims: { C: 1, B: .5, E: .5 }, locs: { pano: svgBayPanorama }, locNames: { pano: '现场全景' }, spots: SP1163.pano, rule: RULE_GIS,
      brief: '任玲玲，116340 地刀的检查情况，向我汇报。设备编号按位报读。',
      goals: [{ id: 'rep', kind: 'say', n: '向监护人汇报 116340 地刀检查结果', pts: .5, re: /./,
...
/* ---------------- 雨淋阀 ---------------- */
const SPRAIN = {
  vg: { pg1: { n: '管网/供水侧压力表', zoom: st => zoomGauge(st, 'pg1') }, pg2: { n: '隔膜/控制腔压力表', zoom: st => zoomGauge(st, 'pg2') }, vOut: { n: '最上方出水阀门', zoom: st => zoomValve(st, 'vOut') }, vIn: { n: '进水阀门', zoom: st => zoomValve(st, 'vIn') }, box: { n: '紧急启动阀盒', zoom: zoomBox } },
...
const EXAM_RAIN = {
  ...
  bg: '#3主变防护区发生火灾，且消防报警主机故障、无法通过主机远程启动。两阶段：雨淋阀正常状态检查 → 火情发生后的机械应急启动。做题与操作交替：值班负责人即时回应；操作关先在屏幕上给出怎么做，做完当场打分；不设消防水泵房检查，不安排水泵控制柜操作。',
  max: 13.5, pass: 8, crit: 7, cover: ['A', 'B', 'D', 'E', 'F', 'G'],
  init(arm) { return { pg1: 'ok', pg2: arm.rainAbn ? 'low' : 'ok', vIn: 'open', vOut: 'open', boxOpen: false, handle: 0, spray: false }; },
  stations: [
    { id: 's0', task: '检查 #3主变雨淋阀组是否处于正常备用状态', guide: [{ g: 'pg1', t: '看管网/供水侧压力表', how: '点阀组左下的压力表，读指针区域与读数' }, { g: 'pg2', t: '看隔膜/控制腔压力表', how: '点阀体左上的压力表' }, { g: 'vOut', t: '看最上方出水阀门阀位', how: '手柄与管道平行为开' }, { g: 'vIn', t: '看进水阀门阀位', how: '阀体下方的蝶阀' }, { g: 'concl', t: '四项都正常后下结论', how: '说：压力正常、阀门开启，阀组处于正常备用状态' }], part: '场景0 · 正常状态检查', title: '#3主变雨淋阀组正常状态检查', dims: { F: 1, B: 1, G: .5 }, locs: { vg: svgValveGroup }, locNames: { vg: '#3主变雨淋阀组' }, spots: SPRAIN.vg, risk: '阀组未处于正常备用状态时，火情发生后雨淋阀无法投入或无法喷水',
    ...
    { id: 'q0', type: 'quiz', part: '场景0 · 答题', title: '答题 · 备用状态', dims: { F: 1, D: .5 }, locs: { vg: svgValveGroup }, locNames: { vg: '#3主变雨淋阀组' }, spots: {},
      brief: '阀组看完了，两道题。',
      items: [
        { id: 'qa1', kind: 'qa', pts: 1, q: '进、出水阀门为什么必须开启？说清水从哪里来、往哪里去。', keys: [{ t: '进水侧供水通路', re: /供水|进水|水源|管网|压力/ }, { t: '出水侧喷雾通路', re: /喷|出水|喷雾|喷头|主变/ }, { t: '启动后才能出水', re: /启动|投入|动作|打开|开启/ }], need: 2, sample: '进水蝶阀开着才有压力水源，出水蝶阀开着雨淋阀启动后水才能送到主变水喷雾管网；任何一个关着，启动了也喷不出水。', sampleBad: '规程要求。', right: '进水侧供水通路和出水侧喷雾通路都要通，雨淋阀启动后才能喷水', why: '两条通路任何一条不通，启动了也喷不出水。', rule: '实操培训方案 · 雨淋阀检查版 · 场景0 正常状态检查', hint: '水从哪里来？往哪里去？' },
        { id: 'f1', kind: 'fill', pts: .5, q: '隔膜式雨淋阀正常备用时，供水侧压力表和___压力表的指针都应在___区。', blanks: [[/控制腔|隔膜/], [/绿/]], sample: '控制腔，绿', why: '两只表分别反映供水通路和隔膜控制腔是否建立正常压力，任何一只指针进红区都不具备投入条件。', rule: '实操培训方案 · 雨淋阀检查版 · 场景0 正常状态检查', hint: '阀体左上那只表叫什么？正常颜色是？' }
      ],
      outro: '好。' },
    { id: 'q1', type: 'quiz', part: '场景A · 答题', title: '答题 · 火情处置', dims: { E: 1, A: .5 }, locs: { pano: svgRainPanorama }, locNames: { pano: '主变雨淋阀区域' }, spots: {},
      brief: '#3主变防护区火警，消防报警主机故障、无法远程启动。先答两道题，再去现场。',
      items: [
        { id: 'c1', kind: 'choice', pts: 1, q: '#3主变防护区发生火灾，消防报警主机故障、无法远程启动雨淋阀，正确做法是？', opts: ['到 #3主变雨淋阀本体处机械手动启动', '先到 #1主变雨淋阀试启动，看系统是否正常', '等消防专业人员到场后再处理', '先关闭出水阀门，防止误喷'], a: 0, why: '报警主机故障时只剩本体机械手动启动一条路，而且必须是着火主变对应的那套阀组。', rule: '实操培训方案 · 雨淋阀检查版 · 场景A', hint: '远程启动不了，还能在哪里启动？' },
        { id: 'c2', kind: 'choice', pts: .5, q: '机械手动启动雨淋阀，操作的是哪个部件？', opts: ['紧急启动阀盒内的手动阀', '电磁阀', '试验阀', '最上方出水蝶阀'], a: 0, why: '电磁阀、试验阀不是紧急启动阀；出水蝶阀一关喷雾通路就断。', rule: '实操培训方案 · 雨淋阀检查版 · 场景C', hint: '贴着「手动应急启动」标签的是哪个？' }
      ],
      outro: '去现场，快。' },
    { id: 'sA', task: '火情发生后找到 #3主变雨淋阀', guide: [{ g: 'find', t: '按现场编号牌确认是 #3', how: '点第三套阀组上方的编号牌，读出编号' }], part: '场景A · 火情发生后正确找到 #3主变雨淋阀', title: '找到 #3主变雨淋阀', dims: { A: 1, E: .5 }, locs: { pano: svgRainPanorama }, locNames: { pano: '主变雨淋阀区域' }, spots: SPRAIN.pano, risk: '火情发生后选择错误主变对应的雨淋阀，会向无火主变喷水而着火主变得不到保护',
    ...
    { id: 'sC1', task: '找到并打开紧急启动阀盒', guide: [{ g: 'box', t: '找阀体右侧贴「手动应急启动」标签的小盒', how: '点开它，按住盒盖直到打开；不要碰进出水蝶阀' }], part: '场景C · 机械手动启动 #3雨淋阀', title: '找到紧急启动阀盒', dims: { A: 1, E: 1 }, locs: { vg: svgValveGroup }, locNames: { vg: '#3主变雨淋阀组' }, sceneOpt: () => ({ fire: true, title: '#3主变雨淋阀组 · 火情处置' }), spots: Object.assign({}, SPRAIN.vg, { vOut: { n: '最上方出水阀门', bad: '停，出水蝶阀不能动，一关喷雾通路就断了。', crit: true, fix: '不得操作与紧急启动无关的主要控制阀', risk: '误操作出水蝶阀会切断喷雾通路' }, vIn: { n: '进水阀门', bad: '停，进水蝶阀不能动。', crit: true, fix: '不得操作与紧急启动无关的主要控制阀', risk: '误操作进水蝶阀会切断供水' }, pg1: { n: '管网/供水侧压力表', bad: '火情发生后不再重复完整检查，直接机械启动。' }, pg2: { n: '隔膜/控制腔压力表', bad: '火情发生后不再重复完整检查，直接机械启动。' } }), risk: '错把其他阀门当作紧急启动阀，机械启动失败或影响系统正常工作',
      brief: (st, mode) => mode === 'exam' ? '消防报警主机故障，在 #3 雨淋阀本体处实施机械手动启动。你来。' : '编号无误，外观也没异常，刚才的状态检查已经确认阀组备用正常，不用再查一遍。报警主机坏了，得在阀本体上机械手动启动。怎么启动，你来。',
      goals: [{ id: 'box', kind: 'op', n: '找到并打开紧急启动阀盒', dev: ['box'], op: 'box:open', re: /阀盒|启动盒|打开.{0,4}盒|紧急启动/, pts: 1, act: st => { st.boxOpen = true; }, praise: '找对了。盒盖打开，里面就是手动阀手柄。', hints: ['机械手动启动的部件在阀本体上。', '找贴着"手动应急启动"标签的小盒。', '阀体右侧那个带盖的小盒，点开它，按住盒盖打开。'], nudge: '在阀体上找，哪个是紧急启动阀盒？' }],
      outro: '' },
    { id: 'sC2', task: '把盒内手动阀操作至全开', guide: [{ g: 'drag', t: '按住黄色手柄，顺时针拖到「全开」再松手', how: '拖到一半不算全开' }], part: '场景C · 机械手动启动 #3雨淋阀', title: '将盒内手动阀操作至全开', dims: { E: 1, D: .5 }, locs: { box: svgBoxOpen }, locNames: { box: '紧急启动阀盒' }, spots: {}, risk: '未将紧急启动手动阀操作至全开，雨淋阀不能可靠启动',
    ...
    { id: 'sD', task: '确认 #3主变水喷雾正常', guide: [{ g: 'spray', t: '看主变四周喷头是否喷水', how: '点主变周围的喷雾区域，说出看到的' }], part: '场景D · 确认 #3主变水喷雾正常喷水', title: '确认 #3主变水喷雾正常', dims: { B: 1, E: .5 }, locs: { tr: svgTransformerSpray }, locNames: { tr: '#3主变本体' }, spots: SPRAIN.tr, enter: st => { st.spray = true; }, risk: '未确认 #3主变水喷雾状态，无法判断处置是否有效',
    ...
    { id: 'sTalk', task: '向值班负责人汇报处置情况', guide: [{ g: 'rep', t: '汇报要素：设备对象、机械手动启动、水喷雾结果', how: '三句话说清' }], part: '汇报', title: '向值班负责人汇报处置情况', dims: { C: 1, E: .5 }, locs: { tr: svgTransformerSpray }, locNames: { tr: '#3主变本体' }, spots: SPRAIN.tr,
      brief: '任玲玲，#3 主变那边处置情况怎么样，向我汇报。',
      goals: [{ id: 'rep', kind: 'say', n: '向值班负责人汇报处置情况', pts: .5, re: /./, keys: () => [{ t: '设备对象（#3主变雨淋阀）', re: /3.?主变|三号主变|#3|＃3/ }, { t: '机械手动启动（手动阀全开）', re: /手动|全开|机械/ }, { t: '水喷雾正常', re: /喷雾|喷水|喷淋/ }], val(t, st) { return this.keys(st).every(k => k.re.test(t)) ? 'ok' : 'bad'; }, sample: '报告，#3主变雨淋阀已在本体处机械手动启动，紧急启动手动阀已全开，#3主变水喷雾系统喷水正常，处置完成。', sampleBad: '已经处理好了。', praise: '收到。#3主变雨淋阀已机械手动启动、水喷雾正常，我通知消防专业到水泵房检查大泵运行。', badText: '汇报要素不完整。', badSay(st) { const miss = this.keys(st).filter(k => !k.re.test(EX.log.filter(m => m.who === 'me').slice(-1)[0].t)).map(k => k.t); return EX.mode === 'exam' ? '汇报要素不完整，再报一遍。' : `还缺：${miss.join('、')}。再报一遍。`; }, right: '设备对象 + 启动方式 + 喷雾结果', hints: ['汇报要说清：哪台主变的雨淋阀、怎么启动的、喷雾结果。', '设备对象、启动方式、结果，三句话。', '照这样报：#3主变雨淋阀已机械手动启动，手动阀全开，水喷雾正常。'] }],
      outro: '这一关到这里。' }
  ]
};
