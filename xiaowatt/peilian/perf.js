...
function perfAdvice(r, vals) {
  ...
  r.groups.forEach(g => {
    ...
    if (!filled.length) { out.push({ t: g.g, s: '本类一项未填', d: '「' + g.items[0].it.n + '」等项目若有业绩，按标准可直接计分，建议核对台账后补报。' }); return; }
  ...
  const zero = r.groups.filter(g => g.sum === 0).map(g => g.g);
  if (zero.length) out.push({ t: '整体', s: '未覆盖的一级指标 ' + zero.length + ' 类', d: zero.join('、') + ' 目前为 0 分。业绩评价分组内合议、存疑修正、组织复评三个环节，没有发文依据的重要业绩事项可走清单制与申报制。' });
