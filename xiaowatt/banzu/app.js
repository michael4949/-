...
function NAV_OF() {
  ...
  const nav = [
    { g: '班组管理' }, { k: 'team', n: '班组画像', ic: 'star' }, { k: 'skills', n: '班员画像', ic: 'hex' }, { k: 'auth', n: '授权认证', ic: 'check' }, { k: 'grow', n: '培养与梯队', ic: 'tree' }, { k: 'perf', n: '绩效与激励', ic: 'gauge' }, { k: 'care', n: '关怀与文化', ic: 'heart', badge: () => careList().filter(c => !LS.get('care_done', {})[c.k + '|' + c.who + '|' + c.when]).length }, { k: 'advise', n: '分析参谋', ic: 'doc' },
...
/* 功能实现状态清单（讲师演示台） */
const STATUS_LIST = [
  ['真实实现', ['状态层流程：派工 → 审票 → 开工 → 回传 → 完工 → 验收，每步写入本机、刷新可回看', '两票 Word / Excel / 文本离线读取与逐项审核、补齐、退回', '派工四条规则校验（证书 / 作业授权 / 工时 / 冲突）与逐人推荐、排除理由', '关键节点周闭环记录、授权模块实操量完工回写', '全站颜色说明：每页底部按模块列出颜色含义；状态色（绿 = 达标、黄 = 需关注、红 = 未达 / 风险）与分类色（蓝 / 紫 / 青等）分开使用，三个班组固定颜色', '作业授权认证表：7 个技能单元 42 个模块逐人逐格确认授权 / 培训中 / 撤销，★按高 / 中 / 初级作业员标注，核心业务自主实施率与★模块断层实时计算，签认与导出 .xls 落本机', '星级班组评价维度初步评分：台账驱动的维度（安全 / 培训 / 台账 / 绩效 / 帮扶 / 关怀 / 作业实施）随操作变化', '绩效系数由履职证据推导，确认、调整、分配表落本机', '班长绩效按员工年度业绩责任书（班长）：考核指标 50% / 重点任务 10% / 综合评价 40% / 加扣分 / 红线；配电自动化班的快速复电成功率取周报，违章、派工、待审票、新增授权取本机台账实时值，确认、下发、改进要求、考评表落本机', '考核指标分分管副总 / 主管 / 班长三个维度，基础值、满分值、挑战值与评分标准取 2026 年度业绩责任书原文，预计得分按责任书评分标准实时计算，督办写入班长通知', '关怀提醒、班组活动、文化活动、督办、轮岗建议、面谈提纲全部写通知或文稿', '跨班组调配：横向对比 → 影响测算 → 发起 → 班长确认 → 返岗评价']],
  ['规则模拟', ['员工画像：综合画像（通用素质模型 5 项，L1–L4）与专业画像（岗位说明书 9 项二级业务，对照本岗级要求判到位 / 未到位），等级依据违章、两票、负责人次数、学时、协同、抢修与缺陷台账', '班组特色标签（专家型 / 骨干型 / 基础型）按技能等级占比判定', '人才断层风险按★模块已授权人数与年龄判定', '培养对象排序按断层★模块、自主实施率、成绩、学时、年龄、员工画像优势加权', '考核指标红黄绿：达到满分值为绿、达到基础值为黄、未达基础值为红；累计类指标按时间进度推算全年', '班长绩效单项得分（上限 120 分）按 ÷1.2 折为百分制后乘权重；综合评价三项（政治过硬 / 勇于创新 / 作风优良）按台账证据给初步分，由部门确认；分档线优秀 90 / 良好 80 / 合格 70', '近五年业务量与人力配置趋势外推', '值班表排班、添加任务推断、文稿逐段生成']],
  ['预设展示', ['考核指标当前值：综合供电可靠率、中压线路故障跳闸次数、第三方客户满意度、安全生产责任制履职评价（局级）与三个班组的中压客户平均停电时间、跳闸次数、停电用户数降幅、安全生产过程管理（试验班、配电运维一班）为预设；配电自动化班与试验班班长责任书指标值暂按配电运行维护一班班长责任书模板；重点任务完成情况、创新成果与加扣分事项为预设', '专业画像暂按运维班高级作业员岗位说明书的 9 项二级业务，配电自动化班岗位说明书待提供后替换；综合画像与专业画像的各人初始等级为预设', '作业授权认证表暂用运维班口径（7 单元 42 模块、★按岗级），配电自动化班专用认证表待提供后替换；各人初始授权状态为预设，原九类核心技能的授权与实操次数已迁移到对应模块', '星级评价中党建 / 标准化 / 定置 / 作业组织等非台账维度的初步得分', '试验班与配电运维一班人员明细、荣誉、稳定性', '敏感岗位任职、考勤异常、离职与借调等团队风险底数', '近 30 天出勤与近四季度成长轨迹', '照片隐患识别、语音转写']],
...
Object.assign(ACT, {
  ...
  'role-menu'() {},
  'role-set'(el) { const r = el.dataset.r; if (!ROLES[r] || r === role()) return; DB.setRole(r); S.briefed = false; XW.clearChat(); location.hash = '#' + ROLES[r].home; render(); XW.answer(r === 'manager' ? '切到管理者了：先看分管副总、主管、班长三个维度的考核指标，再看三个班组的画像、人员、结构、风险和关怀；督办、轮岗、调配都落到班长确认。' : '切到班组长了：先看' + TEAM.name + '的荣誉与星级建设差距，再看班员画像、培养、绩效、关怀。', null, { confirm: false, speak: false }); }
});
