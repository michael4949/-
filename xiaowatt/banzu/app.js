...
const STATUS_LIST = [
  ...
  ['规则模拟', ['员工画像：综合画像（通用素质模型 5 项，L1–L4）与专业画像（岗位说明书 9 项二级业务，对照本岗级要求判到位 / 未到位），等级依据违章、两票、负责人次数、学时、协同、抢修与缺陷台账', '班组特色标签（专家型 / 骨干型 / 基础型）按技能等级占比判定', '人才断层风险按★模块已授权人数与年龄判定', '培养对象排序按断层★模块、自主实施率、成绩、学时、年龄、员工画像优势加权', '年度指标红黄绿按目标值与时间进度判定', '班长绩效五维权重与分档线（优秀 90 / 良好 80 / 合格 70）按主管个人年度业绩责任书分解，另两个班组的违章、安全活动、周报报送、待派待审与本季度关键节点达标周数为预设', '近五年业务量与人力配置趋势外推', '值班表排班、添加任务推断、文稿逐段生成']],
  ['预设展示', ['专业画像暂按运维班高级作业员岗位说明书的 9 项二级业务，配电自动化班岗位说明书待提供后替换；综合画像与专业画像的各人初始等级为预设', '作业授权认证表暂用运维班口径（7 单元 42 模块、★按岗级），配电自动化班专用认证表待提供后替换；各人初始授权状态为预设，原九类核心技能的授权与实操次数已迁移到对应模块', '星级评价中党建 / 标准化 / 定置 / 作业组织等非台账维度的初步得分', '试验班与配电运维一班人员明细、荣誉、稳定性', '敏感岗位任职、考勤异常、离职与借调等团队风险底数', '近 30 天出勤与近四季度成长轨迹', '照片隐患识别、语音转写']],
