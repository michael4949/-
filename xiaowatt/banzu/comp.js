...
const TICKETREV = {
  ...
  fix() { const t = this.t || TICKET; this.applyFix(); const all = LS.get('tickets', {}); all[t.no] = { st: '已审核', ts: Date.now() }; LS.set('tickets', all); if (t.no === TICKET.no) LS.set('ticket', { no: t.no, st: '已审核', ts: Date.now() }); XW.fly('工作票 ' + t.no + ' 已审核 · 已通知' + t.lead); XW.answer((t.missing.length ? '缺的补上了，' : '') + '票已经通过审核，' + t.lead + '那边收到通知。开工前她要在现场再核一遍安措。', null, { confirm: false }); $('#tkbt').innerHTML = '<span class="tag ok">已审核 · ' + h(t.no) + '</span>'; }
};
