/* ===== 状态层：所有页面读这里，所有操作写这里；数字由流程推导，不预设 =====
   任务：待派 → 已派（自动起草工作票）→ 票已审 → 进行中（回传）→ 已完成（记工时、核心技能实操量回写、关联缺陷转待验收）→ 已关闭
   两票：待审 → 已审核 / 已退回 → 已执行；上传的票也进同一张台账
   关键节点：每周按管控周期打勾闭环；跨班组调配：发起 → 待确认 → 已确认 / 已返岗
   工时 / 人员去向 / 通知 / 缺陷状态 / 现场进度 / 周计划 / 月度汇总 / 任务台账 / 操作流水 全部由此推导 */
const DB = {
  /* ---------- 角色（班组长 / 管理者） ---------- */
  role() { return LS.get('role', 'leader'); },
  setRole(r) { LS.set('role', r); },
  ext() { return !!LS.get('ext', false); },
  /* ---------- 任务 ---------- */
  ...
  todayJobs() { return this.jobs().filter(j => !j.added || j.dateIso === TODAY || j.dateIso === '2026-08-10'); },
  dayIdx(iso) { const d0 = new Date('2026-08-10'); const d = new Date(iso); const i = Math.round((d - d0) / 86400000); return i >= 0 && i <= 6 ? i : null; },
  typeOf(j) { if (j.type) return j.type; const t = j.t; return /验收|试验/.test(t) ? '试验验收' : /排查|整改|专项|批量/.test(t) ? '专项' : /投运|光纤|审图|调试申请|新建/.test(t) ? '建设' : /消缺|故障|异常|失败|缺陷/.test(t) ? '消缺' : /培训|考|安全日/.test(t) ? '培训' : '维护'; },
  lineOf(j) { return j.line || (j.t.match(/田寮站 F02|玉律片区|楼村片区|东环益田三公用柜|风景宝明公用柜|宝天桥公用柜|华发民公用柜|光侨路 3# 公用柜|光侨路|东明大道|民生站 F03|主站|全区/) || [j.t.split(' ')[0]])[0]; },
  /* 任务的作业日期（派工单、周计划、台账都用它） */
  whenOf(j) { if (j.dateIso) return j.dateIso + ' ' + (j.time || '09:00'); const w = j.when || ''; const tm = (w.match(/\d\d:\d\d/) || ['09:00'])[0]; const wk = ['周一', '周二', '周三', '周四', '周五', '周六', '周日']; const nx = wk.findIndex(d => w.includes('下' + d)); if (nx >= 0) return '2026-08-' + String(10 + nx).padStart(2, '0') + ' ' + tm; const th = wk.findIndex(d => w.includes(d)); if (th >= 0) return '2026-08-0' + (3 + th) + ' ' + tm; if (/今日|本周五|^\d\d:\d\d$/.test(w)) return TODAY + ' ' + tm; return '2026-08-10 ' + tm; },
  ...
  now() { const d = new Date(); return '08-07 ' + d.toTimeString().slice(0, 5); },
  ...
  STAGES: { 发现: /./, 登记: /^(?!.*(待确认|已撤销))/, 处理中: /处理中|催办|超期|已登记|已入隐患台账|已派|进行中/, 待验收: /待验收|待验证/, 已关闭: /已关闭|已消缺 · 已验证|已验证/ },
  ...
  nextTicketNo() { const n = this.tickets().filter(t => /2026-0807-/.test(t.no)).length + 1; return '配自 2026-0807-' + String(n).padStart(2, '0'); },
  reviewed() { return this.tickets().filter(t => /已审核|已签发|已执行|已退回/.test(t.st)); },
  ticketM() { const r = this.reviewed(); const n = TICKET_M.n.slice(), ok = TICKET_M.ok.slice(); n[7] += r.length; ok[7] += r.filter(t => t.st !== '已退回').length; return { m: TICKET_M.m, n, ok }; },
  /* 派工后按任务类型起草工作票（含常见缺项，等审票补齐） */
  draftTicket(job, rec) { if (job.ticketNo) return this.ticket(job.ticketNo); const test = /试验|验收/.test(job.t); const kind = job.ticket && /操作票|第.种工作票/.test(job.ticket) ? job.ticket : (job.lead || test ? '第一种工作票' : '第二种工作票'); const line = this.lineOf(job); const climb = /杆|FTU|柱上/.test(job.t); const cab = /柜|站|终端|蓄电池|交换机|开关/.test(job.t);
    const t = { no: this.nextTicketNo(), kind, task: job.t, lead: rec.lead, crew: rec.crew.join('、') + (rec.learn ? '、' + rec.learn + '（随队学习）' : ''), time: this.whenOf(job) + ' – 17:00', jobId: job.id, drafted: true,
      measures: test ? [line + ' 开关分闸并挂"禁止合闸，有人工作"标示牌', '电缆两端验电、放电、接地', '试验区设围栏，人员站在安全距离外', '试验完毕充分放电后拆线'] : climb ? [line + ' 开关分闸并挂标示牌', '装设接地线两组', '作业点设围栏，悬挂"止步，高压危险"标示牌', '登杆前检查杆根、拉线'] : cab ? [line + ' 一次设备保持运行，二次室作业', '柜前设围栏并悬挂"在此工作"标示牌', '使用绝缘工具，仪表经校验'] : ['作业点设围栏并悬挂标示牌', '作业前核对设备名称、编号'],
      missing: test ? [] : climb ? [{ item: '验电', why: '装设接地线前应先验电，票面无验电项' }, { item: '接地线位置', why: '写了接地线，未写装在哪一档' }] : cab ? [{ item: '验电', why: '二次室作业前应验明端子无异常电压，票面无验电项' }] : [],
      rules: test ? ['试验前验电、放电、接地', '试验区设围栏，人员站在安全距离外', '试验完毕充分放电并接地后方可拆线', '开关分闸并悬挂标示牌'] : climb ? ['停电作业应在验明无电后装设接地线', '接地线装设位置应在工作票上写明', '作业点应装设围栏与标示牌', '登杆前应检查杆根、拉线及登杆工具'] : cab ? ['二次室作业前应验明端子无异常电压', '作业点应装设围栏与标示牌', '使用合格的绝缘工具与经校验的仪表'] : ['作业点应装设围栏与标示牌', '作业前核对设备名称、编号'] };
  ...
  /* ---------- 关键节点：本周闭环记录 ---------- */
  nodesDone() { return LS.get('nodes', []); },
  nodeDone(id, who) { const a = LS.get('nodes', []); if (!a.includes(id)) a.push(id); LS.set('nodes', a); const n = NODES.find(x => x.id === id); this.log('关键节点', (n ? n.t : id) + ' 本周已清 · ' + (who || (n ? n.a : ''))); },
  nodeUndo(id) { LS.set('nodes', LS.get('nodes', []).filter(x => x !== id)); },
  nodeRate() { const wk = NODES.filter(n => /周|限时|日日清/.test(n.cyc)); const done = this.nodesDone().filter(id => wk.some(n => n.id === id)).length; return { done, n: wk.length, pct: Math.round(done / wk.length * 100) }; },
  /* ---------- 核心技能实操量回写（完工后按工作票累计） ---------- */
  ...
  setTransfer(id, st, extra) { const a = LS.get('transfers', []); const t = a.find(x => x.id === id); if (t) { Object.assign(t, { st }, extra || {}); LS.set('transfers', a); this.log('跨班组调配', t.who + ' → ' + st); } return t; },
  /* ---------- 任务台账 / 月度汇总（推导） ---------- */
  ...
  typeCounts() { const c = {}; this.taskLedger().forEach(t => { c[t.t] = (c[t.t] || 0) + 1; }); return ['建设', '消缺', '维护', '专项', '试验验收', '培训'].filter(k => c[k]).map(k => [k, c[k]]); },
  month() { const ex = this.extraJobs(); const rv = this.reviewed(); const ds = this.defects(); return { jobs: MONTH_BASE.jobs + ex.length, jobsDone: MONTH_BASE.jobsDone + ex.filter(j => /已完成|已关闭/.test(j.st)).length, tickets: MONTH_BASE.tickets + rv.length, ticketsOK: MONTH_BASE.ticketsOK + rv.filter(t => t.st !== '已退回').length, defectsFound: ds.length, defectsClosed: ds.filter(d => /已关闭|已验证/.test(d.st)).length, safetyDays: MONTH_BASE.safetyDays + ((LS.get('docs', {})['safetyday'] || {}).adopted ? 1 : 0), safetyDaysPlan: MONTH_BASE.safetyDaysPlan, incidentStudy: MONTH_BASE.incidentStudy, hoursTeam: MONTH_BASE.hoursTeam, honors: MONTH_BASE.honors }; },
  /* ---------- 操作流水（工作日志、班后会、参谋输出取数） ---------- */
  log(kind, text) { const a = LS.get('log', []); a.unshift({ ts: Date.now(), d: this.now(), kind, text, role: this.role() }); LS.set('log', a.slice(0, 160)); },
  logs() { return LS.get('log', []); },
  /* ---------- 流程动作（页面按钮调用） ---------- */
  dispatch(id, rec) { const j = this.job(id); if (!j) return null; this.setJob(id, { st: '已派', lead: rec.lead, crew: rec.crew, learn: rec.learn, dispatchedTs: Date.now(), reasons: rec.reasons || null }); const t = this.draftTicket(this.job(id), rec); const names = [rec.lead].concat(rec.crew, rec.learn ? [rec.learn] : []); this.notify(names.join(' · '), j.t + ' 派工单', names.length + ' 人已收到', true); this.log('派工', j.t + '：' + names.join('、')); return t; },
  ...
  finish(id) { const j = this.job(id); if (!j || j.st !== '进行中') return null; const who = this.crewOf(j); const hrs = j.h || 6; this.setJob(id, { st: '已完成', doneTs: Date.now() }); this.addHours(who.map(n => ({ d: '8-7', who: n, job: j.t, h: hrs, added: true }))); this.setStatus(who, '在岗'); if (j.ticketNo) this.setTicket(j.ticketNo, '已执行'); const g = this.progOf(j); if (g) this.updProg(g.id, x => { x.pct = 100; x.done = true; x.warn = false; x.ev.push([this.clock(), who[0], '完工，向班长汇报']); }); const linked = this.linkedDefect(j); if (linked) this.setDefect(linked.id, '待验收'); const sk = j.skill || (this.typeOf(j) === '试验验收' ? 's2' : this.typeOf(j) === '专项' || this.typeOf(j) === '消缺' ? 's9' : null); if (sk) this.skillWrite(who, sk, j.t); this.log('完工', j.t + '，' + who.join('、') + '各记 ' + hrs + ' 小时'); return { who, hrs, linked, skill: sk, over: who.filter(n => this.week(n) > WEEK_LIMIT) }; },
  ...
  linkedDefect(j) { if (!j) return null; const ds = this.defects(); if (j.defectId) return ds.find(d => d.id === j.defectId) || null; const key = j.t.replace(/10kV |抢修|消缺|更换|处理|复查|拆除|批量排查/g, '').trim().split(/[ （(]/)[0].slice(0, 6); return ds.find(d => key.length >= 3 && d.t.replace(/\s/g, '').includes(key.replace(/\s/g, '')) && !/已关闭|已验证|已撤销/.test(d.st)) || null; },
  reset() { ['jobs_add', 'jobst', 'hours_add', 'pstatus', 'notices', 'defects', 'defects_add', 'hazards', 'tickets', 'tickets_add', 'ticket', 'prog', 'prog_ov', 'week_ov', 'log', 'dispatch', 'docs', 'mem', 'levels', 'scores', 'grades', 'cases', 'exp', 'duty', 'risk', 'quiz', 'plan', 'sign', 'kbuse', 'nodes', 'skill9', 'skill9n', 'skillw', 'transfers', 'role', 'ext', 'posteval', 'confirm', 'expuse', 'applyq'].forEach(k => LS.del(k)); }
};
const MONTH = new Proxy({}, { get: (_, k) => DB.month()[k] });
PEOPLE.forEach(p => { p.status0 = p.status; });
DB.refresh();

/* 兼容：旧页面里的 MONTH 只读 DB.month()，与台账同源 */
Object.defineProperty(globalThis, 'MONTH', { get() { return DB.month(); }, configurable: true });
