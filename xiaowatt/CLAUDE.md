...
- 项目一「小瓦特·练」AI 智能陪练底座——**9/13 按客户二次修改重构**：主线偏向人力资源（作业授权认证表 → 8 维能力 → 题库考试取证 → 组长培训计划 → 认证表草稿），新增陪练关卡引擎（教练在侧、学员先动：看 / 做 / 说 + 逐级提示 + 红线制止，docs/02 第 45 条）与两份关卡（1163、雨淋阀）、组长工作台；陪练舱 29 项完整票保留，**9/13 三次口径：全过程去点选（准备 / 遥控 / 核对 / GIS / 拟票 / 测验改为口述与按住操作，docs/02 第 46 条）；四次口径：陪练关卡改「做题 + 操作」两种方式（选择 / 填空 / 问答教练即时回应；操作关屏幕给怎么做、做完打分），朗读语音默认关（docs/02 第 47 条）**。口径见 docs/02 第 44–47 条，客户材料解读见 docs/05
...
node v2exam1163.js # 陪练关卡 · 1163（做题 + 操作，15 分制）：考核正确路径 score 15 steps 9 hints 0 / 训练说错+被制止 score 14.7 kinds judge,order hints 1（说错自动给一级提示）red false stopLine true / 考核红线 red true score 0 / 漏项 miss / 刷新后记录 4 / 组长 rows 8 / 任务下发 / 学员待练 / ERR none
...
| avatar.js | 数字人引擎（形象版，9/2 甲方口径：骨骼数字人不可接受）：以教练形象图为本体（陈志远=daozha、林岚=term、周建国=angui，`_hd` 为 512px 版本），逐字视位时钟驱动呼吸/说话节奏/声波/光环，视线跟随鼠标，8 姿态映射为前倾/转头/点头/摇头；**朗读语音 `TTS`（9/13 恢复）**：无 HeyGen 片段时浏览器合成语音朗读，按角色性别选中文音色，口型跟随 `onboundary` 进度、`onend` 收尾、失败退回逐字时钟；`voiceToggle/voiceLabel` 讲师演示台开关（`xwt_voice`），`__DH_MUTE` / `__DH_SPEED<1` 时不出声；接口 speak/setPose/nod/shake/speaking 不变；**被打断的台词一律触发 onEnd（`stopSpeak / destroy`，docs/02 第 48 条）——等这句说完的流程才不会永远卡住**；构造函数可传形象对象（编辑器预览、剧本试演复用），destroy() 停循环并取消朗读 |
...
| app4.js | 准备/五防/收尾、评分与报告（含 genReview 生成式复盘）、讲师演示台、boot（含卡住 24s 监护人主动提醒，__DH_SPEED<1 时停用）；**上岗前准备口述版（9/13）**：`prepSay` 三审 / 着装 / 精神状态按关键词记项（`PREP_KW`，说空话追问、说漏点名），`prepReadRisk` 监护人逐条宣读 12 条风险、操作人口头确认（"全部明白"不算），`prepDone` 自动进五防；`micStart(btn, inp, fallback)` 统一语音输入（联网且 http(s) 打开时浏览器识别；本地文件 / 离线 / 失败时演示识别＝逐字打入当前该说的内容，所有麦克风都必须给 fallback）；`autoDialog` 走 `rcSay → rcExecNow / insSay`，`prepAuto` 口述路径跑完准备 |
...
| exam.js | **陪练关卡引擎 · 做题 + 操作（9/13 四次口径）**：情境 `type:'quiz'` 做题——`examAskItem` 把题目连同选项快捷回复放进教练对话，`examAnswer/examPick` 判定（选择按字母 / 相近度，填空按空位正则，问答按要点正则与 `need`），训练 / 演练答错可再答一次记半分（`EX.qtry`），第二次给答案与依据并 `exErr(choice/fill/ans)`，考核一次作答，`quizStationDone` 一组题打分；没有活动题目时学员作答 / 点场景 / 卡住提醒都先 `examAskItem` 重问；`examMicText` 麦克风兜底；操作关——`exGuideHtml` 屏幕提示面板（`s.task` + `s.guide`：训练 t+how、演练 t、考核只给任务；步骤随 goal 完成打勾），`stationDone` 做完当场一句「本关得分 x/y，扣在：…」（`k:'score'`）；`EX` 状态、`coachSay`（数字人 `EX.dh` 说 + 对话记录）、`examStart/examEnter/examNext/examFinish`；学员三种动作——`examLook(dev)` 走到设备前放大观察（`renderZoom` 带口述输入）、`exOp('dev:action')` 按住进度环执行（`exBindOps`，前置未满足：训练 / 演练制止，考核记红线 + `red.undo` 复位）、`exBindDrag` 拖手柄、`examSay(text, fromZoom)` 口述 / 汇报 / 提问（知识库 `retrieve`）/ 下结论（`needAll` 漏项判定）；`goalDone`（肯定语轮换 + 提示系数 `HINT_F`）、`exHint` 三级提示、`nextGoal`、`stationDone`（outro 后自动进下一情境）、卡住 26 秒教练主动开口；`examSugg` 建议；入口页 `examEntryHtml`、复盘页 `examReviewHtml`（含教练对话回放）；`examAuto(kind)/examRun(plan)` 自动驾驶（`ok / wrong / red`）；本机键 `xwt_exams` |
| exam_defs.js | 两份陪练关卡 `EXAM_1163`（9 情境：3 组题 q0/q1/q2 + 5 关操作 + 1 系统完成，15 分制及格 9，教练 陈志远）与 `EXAM_RAIN`（8 情境：2 组题 q0/q1 + 6 关操作，13.5 分制及格 8，教练 周建国）：做题情境 `items`（choice/fill/qa：q、opts/a、blanks、keys/need、sample/sampleBad、why、rule、hint、pts），操作关 `task` + `guide`（g/t/how）+ `brief`（按模式变化）+ `outro`，`goals` 为学员要自己做到的事（kind look / op / say / qa / drag；`val` 解析口述 → ok / bad / note；`hints` 三级；`after` 前置；`red` 红线；`praise / badSay / missSay` 教练台词，判定反馈照客户方案原文）；`locs` 地点切换；`spots` 设备热区（无关设备 `bad` 台词、`crit` 关键错误）；读数解析 `readOC / readVolt / readPress / readValve`；依据条款取自 STEPS/RISKS 既有 rule 原文；`EXAMS` |
...
- **浏览器朗读语音已实现但默认关**（9/13 用户：口型与内容对不上时配音作用不大）：`avatar.js` `TTS` 保留（按角色性别选中文音色、口型跟随朗读进度），本机键 `xwt_voice` 默认 off，讲师演示台两处「数字人朗读语音 开 / 关」可临时打开。片段到货后片段优先。
