...
node v2exam1163.js # 陪练关卡 · 1163（做题 + 操作，15 分制）：考核正确路径 score 15 steps 9 hints 0 / 训练说错+被制止 score 14.7 kinds judge,order red false stopLine true / 考核红线 red true score 0 / 漏项 miss / 刷新后记录 4 / 组长 rows 8 / 任务下发 / 学员待练 / ERR none
...
P0（9/4 前必须）：清违规文案 ☑ → 项目一工作台 Dashboard ☑ → 项目二高保真原型 ☑。P1：教练中心 ☑、评分复盘页 ☑。P2：成长档案 ☑、教练编辑器 ☑、班组看板 ☑、知识课堂接入 ☑。**9/13 客户二次修改（项目一）☑**：能力模型 / 题库考试 / 1163 与雨淋阀关卡 / 组长工作台 / 偏人力资源首页；**9/13 三次修改 ☑**：全过程去点选（docs/02 第 46 条）；**四次修改 ☑**：陪练关卡改做题 + 操作、朗读语音默认关（第 47 条）；待办：专家审定评分表、外部重绘图到货替换、HeyGen clips、联网浏览器上试听语音识别与合成。逐项细目与验收标准见 `docs/03_任务清单.md`。
