...
export function ClassroomPage() {
  ...
  return (
    <div className="hpage"><div className="ppage">
      ...
      <Hcard t="接入关系" sub="课程 / 题库 / 学员画像 供给 → 陪练；陪练学时 → 成长地图回写 · 点击节点查看接口">
      ...
      <div className="gtwo g32">
        <Hcard t="课程库" sub={`${list.length}/${cs.length} 门 · 点击课程查看章节并继续学习`}><div className="clsearch"><input value={q} placeholder="搜索课程或章节…" onChange={e => setQ(e.target.value)} /><div className="chips">{tags.map(t => <span key={t} className={`chip ${tag === t ? 'on' : ''}`} onClick={() => setTag(t)}>{t}</span>)}</div></div>
