...
const batchesOf = (id: string) => { const n = id === 'p1' ? 6 : id === 'p3' ? 4 : 12; return Array.from({ length: n }).map((_, i) => { const h = hash(id + i); return { no: i + 1, d: `2026-${String(1 + Math.floor(i * 9 / n)).padStart(2, '0')}-${String(6 + (h % 20)).padStart(2, '0')}`, n: id === 'p2' ? 1800 + (h % 900) : 24 + (h % 8), venue: id === 'p2' ? '线上 + 地市局' : ['培训评价中心', '广西电科院', '数字化部实训室'][h % 3], pass: 86 + (h % 12), score: 4.4 + (h % 6) / 10, done: i < n - 1 } }) }
  ...
  return (
    <div className="h-full grid grid-cols-[1fr_1fr_320px] gap-3 p-3 min-h-0">
      ...
      <div className="flex flex-col gap-3 min-h-0">
        ...
        {p.id === 'p3' && <Panel title="历期交付场景"><table className="grid"><thead><tr><th>期次</th><th>提出部门</th><th>场景</th><th>形态</th><th>状态</th></tr></thead><tbody>{[['4', '市场营销部', '高损台区线损归因辅助', '智能体', '已上线'], ['4', '生产技术部', '配网缺陷工单自动归类', '工作流', '已上线'], ['3', '广西电力交易中心', '月度市场分析报告辅助编制', '智能体', '已上线'], ['3', '安全监管部', '两票常见错项自动校核', '智能体', '试运行'], ['2', '物资部', '技术规范书草稿生成', '智能体', '已上线'], ['2', '人力资源部', '培训需求缺口测算', '工作流', '已上线']].map(r => <tr key={r[2]} className="cursor-pointer" onClick={() => push({ v: 'programBatch', id: p.id, no: parseInt(r[0]) })}><td className="num">第 {r[0]} 期</td><td>{r[1]}</td><td className="font-medium">{r[2]}</td><td>{r[3]}</td><td><span className={`tag ${r[4] === '已上线' ? 'tag-ok' : 'tag-warn'}`}>{r[4]}</span></td></tr>)}</tbody></table></Panel>}
        <Panel title={`期次　${batches.length} 期`} className="flex-1" bodyClass="overflow-auto scroll" extra={<span className="text-[10.5px] text-slate-500">点击进入三级</span>}><table className="grid"><thead><tr><th>期次</th><th>日期</th><th>地点</th><th>人数</th><th>通过率</th><th>满意度</th><th>状态</th></tr></thead><tbody>{batches.map(b => <tr key={b.no} className="cursor-pointer" onClick={() => push({ v: 'programBatch', id: p.id, no: b.no })}><td className="num">第 {b.no} 期</td><td className="num">{b.d}</td><td>{b.venue}</td><td className="num">{b.n.toLocaleString()}</td><td className="num">{b.done ? `${b.pass}%` : '—'}</td><td className="num">{b.done ? b.score.toFixed(1) : '—'}</td><td><span className={`tag ${b.done ? 'tag-ok' : 'tag-warn'}`}>{b.done ? '已结业' : '报名中'}</span></td></tr>)}</tbody></table></Panel>
      </div>
      <div className="flex flex-col gap-3 min-h-0">
...
export function ProgramBatch({ id, no }: { id: string; no: number }) {
  ...
  const p = MAIN_PROGRAMS.find(x => x.id === id) ?? MAIN_PROGRAMS[0]
  const b = batchesOf(p.id).find(x => x.no === no) ?? batchesOf(p.id)[0]
  const h = hash(p.id + no)
  const units = BUREAUS.slice(h % 8, h % 8 + 5).map((u, i) => ({ label: u.replace(/供电局$/, ''), v: Math.round(b.n * [.3, .25, .2, .15, .1][i]) }))
  const outputs = p.id === 'p1' ? COURSES.slice(h % 30, h % 30 + 6).map(c => ({ n: c.name, k: '成套课件', st: c.status })) : p.id === 'p3' ? ['高损台区线损归因辅助', '配网缺陷工单自动归类', '两票常见错项自动校核', '技术规范书草稿生成'].map((n, i) => ({ n, k: i % 2 ? '工作流' : '智能体', st: i < 3 ? '已上线' : '试运行' })) : ['本岗位 AI 使用场景（骨干层提交）'].map(n => ({ n, k: '场景', st: `${Math.round(b.n * .82)} 份` }))
  return (
    <div className="h-full grid grid-cols-[300px_1fr_320px] gap-3 p-3 min-h-0">
      <div className="flex flex-col gap-3 min-h-0">
        <div className="panel p-4 shrink-0"><span className="tag tag-gold">{p.badge}</span><div className="text-[16px] font-semibold serif mt-1 leading-snug">{p.name} · 第 {b.no} 期</div><div className="text-[11px] text-slate-500 mt-1">{b.d} · {b.venue} · {b.n.toLocaleString()} 人</div><div className="flex items-center gap-3 mt-3"><Ring v={b.done ? b.pass : Math.round(b.n / (b.n + 6) * 100)} size={64} stroke={7} color={b.done ? 'var(--ok)' : 'var(--ai)'} sub={b.done ? '通过率' : '报名率'} /><div className="text-[11.5px] text-slate-600 leading-relaxed">{b.done ? <>满意度 <b className="num">{b.score.toFixed(1)}</b> / 5<br />产出 <b className="num">{outputs.length}</b> 类<br />结业 <b className="num">{Math.round(b.n * b.pass / 100)}</b> 人</> : <>候补 <b className="num">{6 + (h % 10)}</b> 人<br />开班 <b className="num">{b.d}</b><br />讲师已确认</>}</div></div></div>
        <Panel title="学员构成" className="flex-1"><div className="p-3"><Columns h={110} data={units} /></div></Panel>
      </div>
      <div className="flex flex-col gap-3 min-h-0">
        <Panel title="结业产出" className="flex-1" bodyClass="overflow-auto scroll"><table className="grid"><thead><tr><th>产出物</th><th>形态</th><th>状态</th></tr></thead><tbody>{outputs.map(o => <tr key={o.n} className="row-in"><td className="font-medium">{o.n}</td><td><span className="tag">{o.k}</span></td><td><span className={`tag ${o.st === '已上线' || o.st === '已发布' ? 'tag-ok' : 'tag-warn'}`}>{o.st}</span></td></tr>)}</tbody></table>{p.id === 'p1' && <div className="p-3 hair-t"><button className="btn btn-sm" onClick={() => push({ v: 'lib' })}>在课程库中查看本期课件 ›</button></div>}</Panel>
        <Panel title="评价"><div className="p-3"><Columns h={90} data={[{ label: '内容实用', v: 4.8 }, { label: '讲师', v: 4.7 }, { label: '工具易用', v: 4.5 }, { label: '产出可用', v: 4.6 }, { label: '组织', v: 4.7 }].map(x => ({ ...x, v: b.done ? x.v : 0 }))} max={5} /></div></Panel>
      </div>
      <div className="flex flex-col gap-3 min-h-0">
        <Panel title="AI 评价" extra={<span className="ai-badge">期次</span>}><div className="p-3"><div className="ai-out text-[12px]"><Typewriter text={b.done ? `第 ${b.no} 期通过率 ${b.pass}%，满意度 ${b.score.toFixed(1)}；${p.id === 'p1' ? '产出课件已进入两级审核，其中 4 门已发布到课程库' : p.id === 'p3' ? '交付场景中 3 个已上线，建议把「两票常见错项自动校核」纳入安监陪练科目' : '骨干层提交场景中 12 个与评估表 P1 场景重合，建议转入场景开发训战营'}。` : `第 ${b.no} 期报名 ${b.n} 人，候补 ${6 + (h % 10)} 人；讲师与场地已确认，开班前 3 天推送预习材料。`} speed={7} /></div>
          <div className="mt-2 space-y-1.5"><button className="act-btn" onClick={() => toast('已导出学员名单与产出清单')}><span className="ic">出</span>导出名单与产出<small>xlsx</small></button><button className="act-btn gold" onClick={() => jump('plan', 'track')}><span className="ic">跟</span>查看学员后续计划<small>千人千面 · 跟踪</small></button></div></div></Panel>
      </div>
    </div>
  )
}
