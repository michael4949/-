...
function actionAnswer(q: string, roleId: string, s: Skill): Answer {
  ...
  return { id: `a${++seq}`, q, kind: '动作', text: result, think: [`识别意图：执行动作 · 技能「${s.name}」`, `解析参数：${params.map(p => `${p.k}=${p.v}`).join('，')}`, `权限校验：${s.roles.join(' / ')}`, '执行并回写业务系统'], refs: [], follow: s.id === 'do_coach' ? ['看本班组陪练完成情况', '再下发一次错题重练'] : s.id === 'do_course' ? ['去生成工作台', '相关的中枢条目有哪些'] : ['查看我的待办', '这条任务的完成情况'], actions: s.go ? [{ k: `打开${s.go.node === 'coach' ? '班组看板' : s.go.node === 'factory' ? '生成工作台' : s.go.node === 'plan' ? '计划引擎' : '开班日历'}`, d: '查看执行结果', node: s.go.node, tab: s.go.tab, skill: s.id }] : [{ k: '查看提醒', d: '待办列表', node: 'ask', tab: 'skills', skill: s.id }], skill: s.id, exec: { steps, result, params }, confidence: .95, scope: scopeOf(roleId), ms: 720 }
}
