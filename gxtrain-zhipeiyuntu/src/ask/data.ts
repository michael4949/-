...
export const SKILLS: Skill[] = [
  ...
  { id: 'do_class', name: '预约开班', kind: '执行', desc: '查询开班日历并为个人或班组预约名额，满员时排候补。', params: [{ k: '课程', d: '开班日历中的课程' }, { k: '人数', d: '' }], example: '帮我报名 10-16 的班组长 AI 带班课', roles: ['全部身份'], uses: 3260, ok: 96.8, status: '已上线', go: { node: 'factory', tab: 'system' }, logs: mkLogs('do_class', ['帮我报名岗位 AI 工具入门', '安规复训什么时候开班']) },
...
export const HOTS: Hot[] = ASK_HOT.map((h, i) => {
  ...
  return { rank: i + 1, ...h, units: [pick(BUREAUS, s), pick(BUREAUS, s >>> 3), pick(BUREAUS, s >>> 6)].filter((v, j, a) => a.indexOf(v) === j), roles: i < 5 ? ['一线员工', '班组长'] : ['一线员工', '培训专责'], trend: Array.from({ length: 8 }).map((_, k) => Math.round(h.n / 8 * (0.7 + ((hash(h.q + k) % 60) / 100) + (h.up ? k * .06 : 0)))), samples: [h.q, `${h.q}？现场怎么做`, `新员工问：${h.q}`], sat: 4.2 + ((s % 7) / 10), assetId: ASSET_OF[h.q],
    actions: [{ k: h.act, d: '已由热度触发', st: '已完成', go: h.act.includes('课') || h.act.includes('立项') ? { node: 'factory', tab: 'lib' } : h.act.includes('陪练') ? { node: 'coach', tab: 'plaza' } : { node: 'factory', tab: 'system' } }, { k: '补充助手条目', d: '把高频问法写成标准回答', st: i % 2 ? '进行中' : '待启动', go: { node: 'hub', tab: 'ingest' } }, { k: '生成班前会知识卡', d: '推送到提问集中的班组', st: '待启动', go: { node: 'coach', tab: 'team' } }] }
})
