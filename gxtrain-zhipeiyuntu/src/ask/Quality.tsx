...
export function QualityView() {
  ...
  return (
    <div className="h-full flex flex-col gap-3 p-3 min-h-0">
      <div className="grid grid-cols-5 gap-3 shrink-0">{[['满意度', `${TREND_A[25].sat.toFixed(2)} / 5`, `较上周 ${(TREND_A[25].sat - TREND_A[24].sat >= 0 ? '+' : '')}${(TREND_A[25].sat - TREND_A[24].sat).toFixed(2)}`], ['知识命中率', `${TREND_A[25].hit.toFixed(1)}%`, '数据类 96.8%'], ['出处准确率', '98.1%', '抽检 1,200 条'], ['低分回答', '2.3%', '已全部进入复核'], ['敏感拦截', '38 次', '本月，0 泄露']].map(([k, v, d], i) => <div key={k} className={`hkpi ${i % 2 ? 'gold' : ''}`}><div className="k">{k}</div><div className="v num">{v}</div><div className="d">{d}</div></div>)}</div>
