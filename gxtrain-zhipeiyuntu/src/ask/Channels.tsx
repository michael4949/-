...
export function ChannelView({ id }: { id: string }) {
  ...
  return (
    <div className="h-full grid grid-cols-[1fr_1fr_300px] gap-3 p-3 min-h-0">
      ...
      <Panel title="按单位使用" bodyClass="overflow-auto scroll" extra={<span className="text-[10.5px] text-slate-500">点击进入三级</span>}><div className="p-3 space-y-2">{c.units.map(u => <button key={u.name} className="w-full text-left" onClick={() => push({ v: 'channelUnit', id: c.id, unit: u.name })}><div className="flex items-center gap-2 text-[12px] mb-1"><span className="font-medium">{u.name}</span><span className="ml-auto num text-slate-500">{u.n} / 日</span><span className="text-[var(--gold)]">›</span></div><Progress v={u.n / Math.max(...c.units.map(x => x.n)) * 100} /></button>)}</div></Panel>
