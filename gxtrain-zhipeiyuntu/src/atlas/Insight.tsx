  ...
  return (
    <div className="h-full flex flex-col gap-3 p-3 min-h-0">
      ...
      <div className="flex-1 min-h-0 grid grid-cols-[1fr_1fr_340px] gap-3">
        ...
        <div className="flex flex-col gap-3 min-h-0">
          <Panel title="陪练频次 × 达标率增量　28 个单位" extra={<span className="text-[10.5px] text-slate-500">点击单位看方案落地</span>}><div className="px-2 pt-1"><Scatter points={pts} xLabel="月均陪练场次 / 人" yLabel="达标率较年初增量 pt" h={230} xMin={0} xMax={4.5} yMin={0} yMax={30} sel={sel} onPoint={p => { setSel(p.id); push({ v: 'scenarioUnit', coach, course, mentor, unit: UNITS.find(u => u.id === p.id)!.name }) }} /></div></Panel>
...
export function ScenarioView({ coach, course, mentor }: { coach: number; course: number; mentor: number }) {
  ...
  return (
    <div className="h-full grid grid-cols-[300px_1fr_320px] gap-3 p-3 min-h-0">
      ...
      <Panel title="各单位预测效果　按增量排序" bodyClass="overflow-auto scroll" extra={<span className="text-[10.5px] text-slate-500">点击进入单位落地</span>}>
