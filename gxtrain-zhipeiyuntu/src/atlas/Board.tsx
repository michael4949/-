  ...
  return (
    <div className="h-full flex flex-col gap-3 p-3 min-h-0">
      ...
      <div className="flex-1 min-h-0 grid grid-cols-[320px_1fr_360px] gap-3">
        ...
        <div className="flex flex-col gap-3 min-h-0">
          <Panel title="AI 洞察" bodyClass="overflow-auto scroll" extra={<span className="text-[10.5px] text-slate-500">每周一 06:00</span>}><div className="p-2 space-y-1.5">{AT_INSIGHTS.map(x => <button key={x.k} onClick={() => push({ v: x.go } as Route)} className="a-card w-full text-left"><div className="flex items-center gap-2"><span className={`tag tag-${x.tag}`}>{x.k}</span><span className="num text-[12px] font-semibold" style={{ color: 'var(--indigo)' }}>{x.v}</span></div><div className="text-[11px] text-slate-600 leading-snug mt-1">{x.d}</div><span className="go">›</span></button>)}</div></Panel>
          <Panel title="单位达标率　前 8 / 后 4" className="flex-1 min-h-[170px]" bodyClass="overflow-auto scroll">
          ...
          <Panel title="P1 缺口" bodyClass="overflow-auto scroll"><div className="p-2 space-y-1">{GAPS.filter(g => g.priority === 'P1').map(g => <button key={g.id} onClick={() => push({ v: 'gapDetail', id: g.id })} className="w-full text-left flex items-center gap-2 px-2 py-1.5 hover:bg-slate-50 rounded-lg text-[12px]"><PriTag p={g.priority} /><span className="flex-1 truncate">{g.ability} · {g.line}</span><span className="num text-slate-500">{g.people} 人</span></button>)}</div></Panel>
        </div>
      </div>
    </div>
  )
