...
export function IdpView({ id, unit, team }: { id?: string; unit?: string; team?: string }) {
  ...
  return (
    <div className="h-full grid grid-cols-[290px_1fr_340px] gap-3 p-3 min-h-0">
      ...
      <div className="flex flex-col gap-3 min-h-0">
        <Panel title="12 个月计划排布" extra={<span className="text-[10.5px] text-slate-500">金线为本月 · 点击条目进入二级</span>}><div className="p-3"><Gantt rows={rows} cols={12} colLabel={i => MONTHS[i]} now={NOW_M - .5} onSpan={(r, i) => { const it = items.filter(x => x.kind === r.name)[i]; if (it) push({ v: 'idpItem', pid: p.id, id: it.id, unit: p.unit, team: p.team }) }} /></div></Panel>
