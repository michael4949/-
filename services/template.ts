...
export function drawTopBanner(ctx: Ctx, W: number, H: number, title: string, t: number, sansSerif = TEMPLATE.serif) {
  const bh = H * TEMPLATE.bannerHeightRatio;
  // 暗底
  const g = ctx.createLinearGradient(0, 0, 0, bh);
  g.addColorStop(0, "#0a0a0c");
  g.addColorStop(1, "#17141a");
  ctx.fillStyle = g;
  ...
  for (let i = 0; i < 7; i++) {
    ...
    rg.addColorStop(0, "rgba(120,120,130,0.16)");
    rg.addColorStop(1, "rgba(120,120,130,0)");
    ctx.fillStyle = rg;
    ctx.fillRect(0, 0, W, bh);
  }
  // 底部细分隔线
  ...
  // 标题（衬线、红棕、黑描边、双行居中，压在横幅下半部）
  const fontSize = Math.round(W * 0.072);
  ctx.font = `700 ${fontSize}px ${sansSerif}`;
  ...
  lines.forEach((ln, i) => {
    ...
    ctx.strokeStyle = TEMPLATE.titleStroke;
...
/** 中部网页截图：放进圆角取景框，长截图随进度向上缓滚。 */
export function drawScreenshot(ctx: Ctx, W: number, H: number, img: Bitmap | null, p: number, seed = 0) {
  const top = H * TEMPLATE.shotTopRatio;
  const fh = H * TEMPLATE.shotHeightRatio;
  // 背板
  ctx.fillStyle = "#05070d";
  ctx.fillRect(0, top - 8, W, fh + 16);
  if (!img) {
    // 没截图：暗科技渐变兜底
  ...
  const pad = W * 0.04;
  const boxW = W - pad * 2;
  const fit = boxW / iw;                 // 按宽铺满
  const dispH = ih * fit;
  const shown = Math.min(dispH, fh);
  const x = pad, y = top;
  const overflow = Math.max(0, dispH - fh);
  const scroll = overflow * easeInOut(clamp(p, 0, 1)); // 长图缓滚
  const rad = Math.round(W * 0.028);
  ctx.save();
  roundRect(ctx, x, y, boxW, shown, rad);
  ctx.clip();
  ctx.fillStyle = "#ffffff";
  ctx.fillRect(x, y, boxW, shown);
  ctx.drawImage(img, x, y - scroll, boxW, dispH);
  ctx.restore();
  // 取景框描边
...
export function drawBottomMatrix(ctx: Ctx, W: number, H: number, t: number, seed = 1) {
  ...
  ctx.fillStyle = g;
  ctx.fillRect(0, top - H * 0.04, W, h * 0.34 + H * 0.04);
}
...
export function drawSubtitleBand(ctx: Ctx, W: number, H: number, text: string, sans = TEMPLATE.sans) {
  ...
  lines.forEach((ln, i) => {
    ...
    ctx.strokeStyle = TEMPLATE.subStroke;
    ctx.strokeText(ln, W / 2, y);
    ctx.fillStyle = TEMPLATE.subFill;
    ctx.fillText(ln, W / 2, y);
  });
}
...
/** 一帧完整模板。state 提供该时刻的标题/截图/字幕/进度。 */
export function drawTemplateFrame(
  ctx: Ctx, W: number, H: number,
  state: { bannerTitle: string; shot: Bitmap | null; subtitle: string; sceneProgress: number; t: number; seed: number },
  fonts = { serif: TEMPLATE.serif, sans: TEMPLATE.sans }
) {
  ctx.fillStyle = "#05070d";
  ctx.fillRect(0, 0, W, H);
  drawScreenshot(ctx, W, H, state.shot, state.sceneProgress, state.seed);
  drawBottomMatrix(ctx, W, H, state.t, state.seed + 1);
  drawTopBanner(ctx, W, H, state.bannerTitle, state.t, fonts.serif);
